...
const MembersTab = ({ project, fetchProjects, userId }) => {
  ...
  const handleSaveMember = async (updatedMember) => {
      ...
      try {
        const response = await axios.put(
          `https://amplifybe-2.onrender.com/api/edit-member-role/${project._id}`,
          {
  ...
  const handleRemoveMember = async (memberId) => {
    ...
    try {
      const response = await axios.delete(`https://amplifybe-2.onrender.com/api/delete-member-from-project/${project._id}/${memberId}`);
